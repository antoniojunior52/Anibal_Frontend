...
const NewsFormFull = ({ news, fetchAllData, handleSave, handleDelete, showNotification }) => {
  ...
  const handleEdit = (item) => {
    ...
    setResetKey(prevKey => prevKey + 1); window.scrollTo(0, 0);
  };

  return (
    <FormWrapper title="Gerir Notícias" icon={<FileText className="mr-2 text-teal-500" />}>
      <form onSubmit={handleSubmit} className="space-y-4 mb-6">
        ...
        <button type="submit" disabled={isLoading} className="w-full bg-teal-500 text-white p-2 rounded-md shadow-sm hover:bg-teal-600 flex items-center justify-center space-x-2 disabled:bg-gray-400">
          {isLoading ? <LoadingSpinner size="sm"/> : <><PlusCircle size={18} /><span>{editing ? "Atualizar Notícia" : "Adicionar Notícia"}</span></>}
        </button>
        {editing && !isLoading && <button type="button" onClick={resetForm} className="w-full bg-gray-500 text-white p-2 rounded-md hover:bg-gray-600">Cancelar Edição</button>}
      </form>
      <h3 className="text-lg font-semibold border-t pt-4 mt-6">Notícias Publicadas</h3>
        ...
        {news.length > 0 ? news.map((item) => (
          <div key={item._id} className="flex justify-between items-center bg-gray-50 p-3 rounded-md shadow-sm">
            ...
            <div className="space-x-2 flex items-center">
              ...
              <button onClick={() => handleDelete("/api/news", fetchAllData)(item._id)} className="text-red-500 p-2 rounded-full hover:bg-red-100"><Trash2 size={18} /></button>
            </div>
          </div>
        )) : (
          <p className="text-gray-500 text-center py-4">Nenhuma notícia publicada.</p>
        )}
      </div>
    </FormWrapper>
  );
