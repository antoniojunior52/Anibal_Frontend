...
const HistoryFormFull = ({ history, fetchAllData, handleSave, handleDelete, showNotification }) => {
  ...
  const handleEdit = (item) => {
    ...
    setDescription(item.description); window.scrollTo(0, 0);
  };
    ...
    <FormWrapper title="Gerir História" icon={<Milestone className="mr-2 text-yellow-500" />}>
      <form onSubmit={handleSubmit} className="space-y-4 mb-6">
        ...
        <div className="relative">
          ...
          <label htmlFor="history-description" className={`absolute left-3 text-gray-500 transition-all duration-200 ease-in-out cursor-text ${description ? 'top-[-10px] text-xs text-[#4455a3] bg-white px-1' : 'top-2 text-base'} peer-focus:top-[-10px] peer-focus:text-xs peer-focus:text-[#4455a3] peer-focus:bg-white peer-focus:px-1`}>Descrição</label>
        </div>
        <button type="submit" disabled={isLoading} className="w-full flex justify-center items-center bg-yellow-500 text-white p-2 rounded-md shadow-sm hover:bg-yellow-600 transition-colors disabled:bg-gray-400">
      ...
      <h3 className="text-lg font-semibold border-t pt-4 mt-6">Marcos Históricos</h3>
      <div className="space-y-3 max-h-96 overflow-y-auto pr-2 mt-4">
        {history.length > 0 ? history.map((item) => (
          <div key={item._id} className="flex justify-between items-center bg-gray-50 p-3 rounded-md shadow-sm">
            <div><span className="font-semibold">{item.year}: {item.title}</span></div>
            <div className="space-x-2 flex items-center">
              <button onClick={() => handleEdit(item)} className="text-blue-500 p-2 rounded-full hover:bg-blue-100"><Pencil size={18} /></button>
              <button onClick={() => handleDelete("/api/history", fetchAllData)(item._id)} className="text-red-500 p-2 rounded-full hover:bg-red-100"><Trash2 size={18} /></button>
            </div>
          </div>
        )) : (
          <p className="text-gray-500 text-center py-4">Nenhum marco histórico adicionado.</p>
        )}
      </div>
    </FormWrapper>
  );
